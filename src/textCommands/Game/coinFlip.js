...
module.exports = {
     ...
     callback: async (client, message, args) => {
          ...
          const result = rand == choose ? await client.addTien(author, soTien * 2) : await client.truTien(author, soTien); 
          ...
          await message.reply(`<a:coin_flip:1163073855540187177> Bạn cược **${soTien}** và chọn **${statusCoin[choose]}**...`)
               .then(msg => {
                    setTimeout(() => {
                         msg.edit(`Đồng xu **mặt ${statusCoin[rand]} và bạn ${rand == choose ? `thắng ${soTien * 2}` : 'mất hết rồi...'}** `);
                    }, 3000);
               })
     }
}